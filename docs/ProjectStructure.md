  ...
  src/
    gui/
      windows/
        ...
        tests/
          gui_feature_controller_tests.cpp
...
tray/bridge/maintenance host 和 `tray_app.*`。旧 Win32 窗口源码已删除，不存在绕过体量检查的 UI 先例。
...
`0.8-D` 已将旧 Win32/GDI+ 窗口移出生产 target，`0.8-E` 在功能、视觉和并发验收后删除其源码、theme、
control id 与 HWND 自动化。当前 Qt GUI 已完成功能对等，但仍需 `0.8-F/G` 才成为 0.8 正式发行物。
...
connection/server、snapshot bridge 和 maintenance endpoint 继续按资源所有权拆分。Windows UI 只有 Qt
sidecar 一条生产与源码路径，不保留旧窗口 fallback。
...
session、轻量 destroy、graceful quit，以及 GUI 反复 hide/activate 时 8/16 路 SSE 的精确字节、chunk 顺序、
长度、结束标记和零 GUI runtime diagnostic。旧 HWND 自动化已删除。
