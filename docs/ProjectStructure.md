...
不链接共享 core。`0.8-C` 已把共享 wire codec、tray named-pipe server、launcher/session 和 maintenance
endpoint 接好，但生产 tray 尚不启动 prototype；`0.8-D` 接入 Qt client/lifecycle，`0.8-E` 功能对等验收
后才删除旧 Win32/GDI+ 窗口。当前 production 行为仍以 0.7.0 为准。
...
`tray_app.*` 只保留宿主/message-loop 编排并把共享 ViewModel callback 派发回 UI thread。icon/menu、runtime
command/exit、GUI IPC host 分别位于 `tray/`、`tray_runtime_host.cpp` 与 `tray_ipc_host.cpp`；GUI launcher、
session、named-pipe connection/server、snapshot bridge 和 maintenance endpoint 继续按资源所有权拆分。
