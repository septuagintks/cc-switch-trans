...
- [ ] `tools/check_stage13_prerequisites.sh` 完整通过；当前仅完整 Xcode 版本项失败，ad-hoc
  打包不要求 Developer ID、notarytool 或 stapler。
...
- [x] 发布策略明确不执行 Developer ID、notarization、staple 或 `spctl`，不声明 Gatekeeper
  信任。
- [x] 正式名称 ZIP 的固定白名单、内部 checksum、arm64 slice 和签名验证通过。
