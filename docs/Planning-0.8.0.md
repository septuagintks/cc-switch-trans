...
| `0.8-E` 状态 | 完成；Qt 功能对等、错误交互、视觉验收与旧 Win32 GUI 删除 |
| 当前工作包 | `0.8-F`：descriptor-driven Rule Builder、preview 与 undo/redo |
...
| `0.8-E` | 0.7 功能对等、错误 dialog、migration UI、视觉/动画与资源测试 | 完成：Qt GUI 人工验收通过，旧 Win32 GUI 已删除 |
...
**`0.8-E` 执行结果**：Profiles、Rules、Settings、服务控制、dirty-close、应用内错误 dialog 和
storage migration/replace 二次确认均已接入 Qt sidecar。Profile Save 在 snapshot/result 任意到达顺序下
按字段 rebase，验证、stale 或 runtime apply 失败保留本地输入；窗口关闭会等待服务端 pending command。
字段编辑器、下拉框、滚动条、开关和页面使用稳定布局与 Reduce Motion，当前下拉选中项保持二级高亮，
实时 hover 使用一级高亮且不保留 last-hover。

Windows 人工视觉验收已通过。自动验证通过 runtime warnings-as-errors `32/32`、Qt warnings-as-errors
`8/8`、production QML probe、结构检查以及真实双构建树联调；联调在 GUI 反复 hide/activate 期间验证
8/16 路 SSE 的 Content-Type、完整字节、chunk 顺序、长度和 `[DONE]`，并要求 GUI stdout/stderr 零运行时
诊断。旧 `src/hosts/windows/main_window.*`、`windows_theme.*`、Win32 control id 和 HWND 自动化脚本已删除，
提取包生命周期验证入口已切换到 Qt sidecar；正式 Qt package whitelist 与 setup 仍由 `0.8-G` 完成。
...
旧 Win32 GUI 已在 `0.8-D` 从生产 target 移出，并在 `0.8-E` 人工、资源和并发验收通过后从源码、资源
常量与自动化中删除。当前没有运行时 fallback；后续源码、portable ZIP 和 setup 均不得重新引入旧 GUI
或不可达 GDI+ theme 代码。历史 `0.7.0` 包只作为外部行为 oracle。
...
`0.8-A` 至 `0.8-E` 已完成，当前从 `0.8-F` 开始实现可视化 Rule Builder、preview 和 undo/redo；
`0.8-G` 继续按已冻结的非商业 Inno Setup 路径实现正式 setup，并补完 maintenance wait/退出时序。
