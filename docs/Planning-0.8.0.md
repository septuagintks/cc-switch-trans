...
| 当前工作包 | `0.8-C`：Windows tray 分层与 `gui-ipc/v1` |
...
| `0.8-C` | `gui-ipc/v1`、named pipe、tray 分层、launcher/session/router | 当前：fake client 可完成全部命令与重连 |
...
`0.8-B` 已完成，当前从 `0.8-C` 开始 tray 分层与 `gui-ipc/v1`；`0.8-G` 继续按已冻结的非商业
Inno Setup 路径实现正式 setup。
