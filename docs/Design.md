...
当前仍保留同步 worker 模型。阶段 9 先建立 benchmark 和资源指标，再实现客户端取消传播与分阶段 timeout；配置持久化、tray 和 macOS 继续后置。
...
- benchmark、客户端取消传播和进一步连接策略优化。
- 配置文件持久化与热重载。
- Windows 托盘图标、菜单、开机自启、后台宿主和双击隐式启动。
- macOS transport、菜单栏图标、登录时启动和发布包。

这些后续功能已经影响当前接口设计，但不反向扩大 `0.2.0` 的协议改写范围。
...
`0.2.0` 已验证 SSE 实时转发、序号 chunk 日志和去聚合。阶段 9 在不改变这些语义的前提下补齐客户端取消传播。
...
当前仍存在需要在阶段 9 测量或补齐的边界：

```text
SSE 生命周期占用同步 worker
客户端断开向上游传播取消
分离连接、发送、响应头、流空闲和总超时
日志队列高水位、背压和 WinHTTP 行为指标
8/16/50 路可重复 benchmark
```

性能负载口径已经确定：`8–16` 路并发 SSE 是桌面常规负载，`50` 路是压力测试，不是常规 SLO。当前继续使用同步 worker；只有基线证明 8–16 路常规负载存在不可接受的排队或资源占用，才进入完整异步 I/O 设计。50 路压力结果不能单独触发网络栈重写。
...
阶段 9 不混入 tray、持久配置或 macOS transport。只有 benchmark 证明同步模型在 8–16 路常规负载下构成主要瓶颈，才单独规划完整异步 I/O 重写。
