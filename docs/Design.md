...
当前 session 使用 `WINHTTP_ACCESS_TYPE_DEFAULT_PROXY`。这只能代表当前实现基线，
不能保证跟随 Windows 当前用户的系统代理。阶段 11 首个网络工作包将其切换为
Windows 11 21H2 上的自动系统代理模式，并验证：系统明确 direct 时可以直连；系统
已经选择代理而该代理失败时不得回退 direct；不支持代理账号密码；运行中修改系统
代理后，新请求使用新设置。
...
- 当前 WinHTTP 使用 default proxy，自动跟随 Windows 当前用户系统代理尚未实现。
