...
`0.8-A` 已加入不会进入请求热路径的独立 `ccs-trans-gui.exe`。它使用 Qt 6.10.3/官方 MinGW 13.1，
...
macOS native main window -> shared presentation contract -> editing/controller services
...
| 模块 | 当前职责 | 禁止承担 |
| --- | --- | --- |
| `src/hosts` | v2 CLI、Windows tray/GUI bridge、macOS menu/window、平台操作、退出码、服务启停 | 路由、规则或 transport 逻辑 |
...
`0.8-C/D` 的 Windows 进程边界固定为：tray 独占 repository、ViewModel、ApplicationController 和代理
...
tray 只在首次 `LoadDraft` 命令完成且状态 revision 为正后启动 GUI；失败完成也可进入 GUI，避免 migration
故障把恢复界面永久挡在进程门外。Qt client 的首个有效 snapshot 自动显示窗口一次，之后普通关闭只隐藏，
必须由 tray Activate 再显示；轻量关闭或隐藏后启用轻量模式会退出 GUI。post-auth 断线不复用一次性 token，
下一次 Open 由 tray 启动新进程和新 session。GUI 不可见时不使用周期 repaint 或状态轮询。
...
Windows 主窗口由独立 Qt Quick 进程持有，不拥有 runtime、repository 或 SQLite。tray 菜单、tray 双击与
第二实例通知只启动或激活 sidecar；关闭主窗口不会停止 listener。普通模式隐藏并复用同一进程，轻量模式
