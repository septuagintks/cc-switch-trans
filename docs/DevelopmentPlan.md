...
实现状态：本机自动验证已完成，VM 手工矩阵待执行。Release 与 warnings-as-errors 构建的
CTest 均为 11/11，通过协议集成、隔离配置下的 tray Start/Stop/Reload/单实例/退出集成、
真实 notification-area icon 集成和 0/1/8/32 Rule microbenchmark。测试脚本默认不修改真实
HKCU startup 或系统代理；两个高风险矩阵都要求显式 confirm 参数并留给 disposable VM。
...
实现状态：`0.5.0` Windows x64 候选包脚本和独立验包脚本已完成。验包从 ZIP 解压到临时
目录后重新检查精确白名单、两个 SHA-256、CLI/资源版本，并运行提取后的 tray 生命周期
集成；脚本兼容 Windows PowerShell 5.1。最终发布判定仍等待
`docs/WindowsValidationChecklist.md` 的 VM 手工结果。
