...
实现状态：共享/macOS 工程实现已在 `dfcea1d` 完成。当前机器完成 Release 与
warnings-as-errors clean build、两套 10/10 CTest、共享协议集成、process proxy 矩阵、AppKit
host/单实例/退出 smoke、Mach-O 依赖审计，以及正式文件名的 ad-hoc 固定白名单 archive 验证。
用户随后授权阶段 14 第 1、2 项的缩短范围：短负载/Rule matrix、15 分钟 mixed soak 和 30 分钟
idle；原 2 小时/8 小时和其余阶段 14 项目仍未执行。完整命令与逐项状态记录在
...
共享 fixture、CLI/menu smoke 和包内容审计已通过。阶段 14 第 1 项短负载/Rule matrix 与第 2 项
缩短的 15 分钟 mixed、30 分钟 idle 已通过；长期原时长、登录项/Finder/Terminal 环境对比与
干净账户候选 smoke 尚未执行。
...
本轮只执行第 1、2 项的 macOS 范围，且经用户授权将第 2 项缩短为 15 分钟 mixed 与 30 分钟
idle。`c8043e3` 的 Release 负载矩阵全部零失败；mixed-16 的两组 Usage 都在 SSE 期间完成，
stress-50 的 32 worker/queue/fd/logger 保持有界。15 分钟 mixed 完成 133 轮、2,128 条 SSE 和
两组各 1,596 次 Usage，后半程 RSS/fd/thread 均未持续增长；30 分钟 AppKit idle 的 runtime 与
host log 均为 `0 bytes` 增长。该结果不替代原 2 小时 mixed、8 小时 idle，也不覆盖第 3-7 项。
