...
本地 dirty edit buffer、首次 draft/revision 门控和独立进程生命周期；生产 tray 已移除旧 Win32 GUI 编译路径，
不提供 fallback。当前开发入口为 `0.8-E`：恢复 0.7 功能对等、应用内错误交互和完整视觉/动画验收。
...
4. 完成 `0.7.0` Windows GUI 功能对等后，删除旧 Win32/GDI+ `main_window.*`、`windows_theme.*` 和
...
之后按需启动或激活 `ccs-trans-gui.exe`；加载失败状态也可进入 GUI，以便 `0.8-E` 承载 migration/recovery。
...
覆盖。warnings-as-errors 已通过 runtime `32/32`、Qt `6/6` CTest，真实双构建树联调覆盖第二实例、crash
relaunch 与 graceful quit。旧 `main_window.*`/`windows_theme.*` 仅保留源码和 0.7 包作 oracle，不参与生产 target。
