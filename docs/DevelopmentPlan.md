...
阶段 11.8 已完成。worker 在读取 HTTP 请求前只获取一次 RequestGeneration，请求限制、
RouteTable、pipeline、Proxy 与 Logger 全程来自同一 generation。热交换日志使用进程内
单调 `generation_id`，swap 事件记录 `previous_generation_id`；request、Usage、Rule、
upstream、SSE chunk、response 和 error 均携带 generation id。

Profile、route、upstream、rule 顺序、timeout、body limit/body logging 与日志路径可对
新请求热切换；listener、worker、metrics reporter 和同路径 writer 参数变化要求 graceful
restart。候选日志路径打开失败不替换现役 generation。CLI 保存 enabled Profile 候选前
运行完整 RuntimeCompiler，route collision 与保留路径冲突不会写入配置。

logger 增加显式 drain 与 active-writer 指标。旧/新日志路径并存时，旧 writer 退出或
候选 writer 失败不会把现役 writer 标为不健康；`server_start` 在启动成功回调前落盘，
退出前 drain，durability failure 触发停止和非零退出。Usage 转发失败的 summary 现在
明确记录 `forwarded=false`，且继续不记录 headers/query/body/client IP。

新增 reload 集成测试同时固定 A/B upstream、local route、Rule 顺序、日志路径、旧请求
所有权、候选冲突/日志失败回滚、ConfigStore 失败后字节不变和 generation 日志链。
Debug 与严格告警构建均为 7/7 CTest，完整 Python 单监听集成通过。

Release 回归中 `smoke`、`desktop-8`、`mixed-16` 均零失败、零 writer failure、零
backpressure；附加 TTFB p50 分别为 `0.442 ms`、`6.030 ms`、`7.486 ms`。常规负载
仍低于阶段 11 冻结基线 `10.619 ms` / `9.474 ms`，未触发性能阻断门槛。原始结果只
保存在 ignored `benchmark-results/stage11.8-current.json`。
