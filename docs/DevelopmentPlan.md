...
矩阵、AppKit host/单实例/退出 smoke、Mach-O 依赖审计，以及 ad-hoc 固定白名单 archive
smoke。完整命令与逐项状态记录在 `docs/MacOSValidationChecklist.md`。
...
6. 正式 `ccs-trans-0.5.0-macOS-arm64.zip` 只由 Developer ID + notary 路径生成。无发布身份时
   仅生成明确带 `-adhoc-smoke` 后缀的结构验证包，不冒充候选发行物。

当前外部阻塞：active developer directory 是 Command Line Tools，机器上没有完整 Xcode.app；
keychain 中没有 Developer ID Application identity。因此完整 prerequisite、Developer ID
codesign、notarization、staple、Gatekeeper、真实登录项 mutation 和相应 Finder/登录环境手工
证据未完成。代码与自动化已就绪，但 `0.5.0-macOS-arm64` 正式候选不能在这些外部条件满足前
声明发布完成。
...
对比、Developer ID/notary 与干净账户正式候选 smoke 留在外部阻塞解除后的发布验收；阶段 14
负载、2 小时 mixed soak 和 8 小时 idle 本轮明确不执行。
