...
本文是 `0.7.0` 的冻结实施合同与过程记录，不再维护当前开发状态。`0.7-A` 至 `0.7-G` 已完成，
正式源码固定为 `0.7.0`、`ccs-trans.config/v3` 与 SQLite schema v1；最终验证、发行物和明确未执行项
以 [Release-0.7.0.md](Release-0.7.0.md) 为准。

| 项目 | 当前值 |
| --- | --- |
| 开发基线 | `0.6.0` tag，commit `f9fe5a8868c46bc7b982995cca03432ed96e27eb` |
| 最终收尾侧 | Windows |
| macOS 范围 | 平台实现、构建、集成、AppKit 对等布局与正式包验证 |
| 目标发行包 | `ccs-trans-0.7.0-Windows-x64`、`ccs-trans-0.7.0-macOS-arm64` |
| 最终版本号 | `0.7.0`；开发期 `-dev` suffix 已在正式源码提交移除 |

最终实现包括进程级 512 MiB budget、
...
随后完成 logger 审计修复，并再次通过两套完整构建、两套 `25/25` CTest、shared integration 与
tray/main-window integration。`0.7-F` 的三视图、完整字段/Rule 文本编辑和 Windows 轻量圆角主题已在
2026-07-18 完成用户验收。

`0.7-A3` 的 macOS platform-local cURL 硬化、严格构建与默认环境测试已在后续回交中关闭；`0.7-G`
AppKit 三视图、响应式布局、原生主题和资源门禁最终落在 `7132053`，并由用户验收。本文后续保留的
中间 blocker、checkpoint 和性能数字只用于解释实施取舍，不代表当前待办。
...
fault target 需要根 CMake 接线，留给 Windows/shared 集成。整个 `0.7-A` 仍保持 open：两平台还需在
SSE headers 前统一最小 chunk allowance。logger 共享缺口已关闭：writer 在发布 `flushed_sequence_`
前释放 batch string/lease，budget、render、lease growth 与 queue allocation 拒绝会通过不持锁的有界
failure callback 触发受控停机，不再静默丢记录或出现 release-before-free 窗口。大量 header 的 shared
filter O(n^2) 只保留为有 profiler 证据后再做的优化；没有证据支持现在改 byte-view 或 polling。
...
- Win32 主窗口已完成 Profiles、Rules、Settings 三视图；Profile stable key、typed application/Profile
  字段、canonical Rule 文本、Format/Update、Apply/Discard/Reload 均通过共享 ViewModel 和
  `ConfigurationEditor`，平台 view 不直接访问 SQLite 或 JSON DOM；
- Windows 视觉层使用集中 palette/metrics 与 GDI+ 抗锯齿圆角，覆盖导航、按钮、开关、输入框、
  Profile selection、状态面、下拉菜单和滚动条；light/dark/high-contrast 仍从 Windows 主题派生；
- Settings 使用“裁剪 viewport + 单一内容容器”，以完整字段行作为滚动边界，16 ms ease-out 只移动内容
  容器；滚动中间帧、快速滚动、顶部/底部稳定帧均未再出现残影、缺字或布局漂移；
- Rules 编辑器不显示白色原生滚动条，发生滚动时短暂显示主题色位置指示器；Profile selection 作为
  非阻塞 UI 过渡，不再通过全局 disable/enable 让无关静态按钮和开关文字闪烁；
- Windows Release + warnings-as-errors 构建通过，CTest `25/25`、tray/main-window integration passed，
  其中包含 16 路并发 SSE、GUI 生命周期和资源检查；2026-07-18 的逐项 GUI 人工验收通过。

更宽的 100%/150%/200% DPI、Narrator、128 Profile 和长时间 GUI idle 组合矩阵仍在 `0.7-H` 候选阶段
统一执行；本工作包完成的是实现冻结与当前 Windows 环境验收，不把未执行的候选矩阵写成 passed。
...
`0.7-G` 以已验收 Windows 窗口作为布局基准：首屏的 header、左侧 Profiles/Rules/Settings 导航、
右侧内容区、底部 draft actions，以及各视图内字段顺序、主要操作位置、信息密度和滚动范围保持对等。
这是一份信息架构与布局比例合同，不是逐像素复制 Win32。macOS 继续使用现有 AppKit 原生主题、字体、
NSControl/NSTextView/NSScrollView、系统 focus ring 和原生滚动动画，不移植 Windows palette、GDI+
owner-draw、圆角半径或自绘滚动条。功能上补齐 0.7 新增字段、Rule 文本和 repository 状态，并继续遵守
native appearance、Retina、Reduce Motion 与 accessibility 合同。
...
- [x] macOS 在合并后提交上复验正式 warnings 与默认环境 CTest，关闭 `0.7-A3`；
...
- [x] 修复 logger rejection/drain ordering，并重跑两套完整构建、两套 `25/25` CTest 与 shared/tray integration；
- [x] 完成 `0.7-F` Windows 三视图、完整字段/Rule 文本编辑和轻量圆角主题；
- [x] `0.7-F` Release + warnings-as-errors build、CTest `25/25`、tray integration 与逐项 GUI 人工验收通过；
- [x] 完成 `0.7-G` AppKit 对等实现，布局以已验收 Windows 窗口为基准，主题保持 macOS 原生；
- [x] Windows 接回主开发权并完成正式源码冻结、候选门禁与发布文档；
- [ ] 双平台 ZIP、checksum 与签名 tag 的外部证据不写回本实施计划，以发行 tag 和交接记录为准。
