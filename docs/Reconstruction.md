...
1. session 使用 `WINHTTP_ACCESS_TYPE_AUTOMATIC_PROXY`，读取当前用户 Windows 系统
   代理、PAC/WPAD 和 bypass 结果；不读取 `HTTP_PROXY`/`HTTPS_PROXY` 环境变量。
...
5. 系统代理在进程运行期间发生变化时，后续新请求应使用新结果，不要求重启服务；
   已经开始的请求不迁移代理。
6. 日志记录 `proxy_mode=windows_system`、是否由系统选择代理、错误阶段和 WinHTTP
   error code，但不记录代理凭据、PAC 内容或带 userinfo 的 URL。

当前实现的 WinHTTP session 仍使用 `WINHTTP_ACCESS_TYPE_DEFAULT_PROXY`，它不能保证
跟随当前用户系统代理。该差异必须在通用配置重构前作为独立小工作包修正和验证。
